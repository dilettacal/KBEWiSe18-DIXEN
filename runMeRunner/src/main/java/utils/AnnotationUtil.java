...
public class AnnotationUtil {
	 ...
	 */
	public static boolean analyzeClass(String classToOpen, String filename)
	...
	public static void writeMethodsToFile(ArrayList[] allMethods, String filename) {
		...
		try {
			...
			/*
			 * Nicht meh notwendig
			 * //bw.write("(Grund: Parameter in der Methodendeklaration)"); bw.newLine();
			 * for(String m: methodsWithRunMesAnnosNotRunnable2) { bw.write(m);
			 * bw.newLine(); }
			 */
			bw.newLine();
		} catch (IOException e) {
			...
			for (Method m : methods) {
				...
				if (methodAnnos.length == 0)
					...
					try {
						...
						Object[] objs = params;
						/*
						 * Soll nicht von uns geworfen werden! if(params.length > 0) { reasonNotIvoke =
						 * 'p'; throw new InvocationTargetException(null, "nicht invokierbar"); }
						 */
						m.invoke(obj, objs);
						 ...
						 * e.getClass().getSimpleName()); else if(reasonNotIvoke == 'e')
						 * methodsNotRunnableException.add(m.getName()+ ": " +
						 * e.getClass().getSimpleName());
						 */

						// Ausgabe in der Form: 'mehtodX: IllegalArgumentException'
						methodsNotRunnableException
								.add(m.getName() + ": " + e.getClass().getSimpleName() + " <- " + e.getCause());
						methodsWithRunMesAnnos.add(m.getName());
					}
